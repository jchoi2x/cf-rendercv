...
export class TypstCompilerDo extends DurableObject<Env> {
  ...
  private readonly manager: TypstCompilerManager = new TypstCompilerManager();
  ...
  private initServer() {
    ...
    this.app.post("/api/v2/generate", async (c) => {
      try {
        ...
        const source = await this.buildTypstSource(payload);
        // return c.json({ ok: true, source });
        return await this.manager.compilePdfResponse(5, source);
      } catch (error) {
      ...
      design: {
        ...
        entries: {
          date_and_location_width:
            design.entries?.date_and_location_width ?? "4.15cm",
          side_space: design.entries?.side_space ?? "0cm",
          space_between_columns:
            design.entries?.space_between_columns ?? "0.3cm",
          ...
          highlights: {
            ...
            nested_bullet: design.entries?.highlights?.nested_bullet ?? "•",
            space_left: design.entries?.highlights?.space_left ?? "0cm",
            space_above: design.entries?.highlights?.space_above ?? "0.15cm",
            space_between_items:
        ...
        templates: {
          education_entry: {
            degree_column:
              design.templates?.education_entry?.degree_column ?? true,
          },
        },
      },
      locale: {
  ...
  private normalizeEntry(entry: unknown): unknown {
  ...
  private buildSections(payload: Record<string, any>): RenderSection[] {
    ...
    if (Array.isArray(cv.rendercv_sections)) {
      return cv.rendercv_sections.map((section: any) => ({
        ...
        entries: Array.isArray(section.entries)
          ? section.entries.map((entry: unknown) => this.normalizeEntry(entry))
          : [],
      }));
    }
    ...
    for (const [title, values] of Object.entries(sectionMap)) {
      ...
      sections.push({
        ...
        entries: rawEntries.map((entry) => this.normalizeEntry(entry)),
      });
    }
    return sections;
  }
  ...
  private async buildTypstSource(payload: unknown): Promise<string> {
    const safePayload =
