...
const getConnections = (defaults: Connections) => {
  return z.object({
    phone_number_format: z
      .enum(["national", "international", "E164"] as const)
    ...
    space_between_connections: z
      .string()
      .default(defaults.space_between_connections ?? "0.5cm")
      .describe(
        "Horizontal space between contact items. The default value is `0.5cm`.",
      ),
  });
};
...
export const getHeader = (defaults: Header) => {
  return z.object({
    alignment: z
      .enum(["left", "center", "right"] as const)
      .default(defaults.alignment ?? "center")
      .describe(
        "Header alignment. Options: 'left', 'center', 'right'. The default value is `center`.",
      ),
    photo_width: z
      .string()
