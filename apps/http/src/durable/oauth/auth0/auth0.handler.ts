...
function normalizeAuth0Scope(scope: string | undefined) {
  ...
  const parts = raw.length ? raw.split(/\s+/g) : [];
  const set = new Set<string>(parts);
  for (const s of REQUIRED_OIDC_SCOPES) set.add(s);
...
export async function authorize(
  ...
  if (
    ...
    const urlStr = as.authorization_endpoint;
    if (!urlStr) {
      return c.text("Invalid authorization endpoint", 500);
    }
    const authorizationUrl = new URL(urlStr);
    ...
    // Our callback requires an OIDC `id_token`, which Auth0 only returns when `openid` is in scope.
    authorizationUrl.searchParams.set(
      "scope",
      normalizeAuth0Scope(c.env.AUTH0_SCOPE),
    );
...
export const Auth0Handler = new Hono<{
  Bindings: Env & { OAUTH_PROVIDER: OAuthHelpers };
}>();

Auth0Handler.get("/authorize", authorize);
Auth0Handler.post("/authorize", confirmConsent);

Auth0Handler.get("/auth/callback/auth0", callback);
Auth0Handler.get("/callback", callback);
